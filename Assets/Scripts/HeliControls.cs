...
{ 
    ...
    private float horizontalInputYaw;

    //Mängu sündmused toimuvad siin
    private void Update()
    {
        //Liigutame  helikopteri üles
        transform.position += transform.up * thrust * Time.deltaTime;//Viimane on seal, et kõigil oleks ükskõik mis arvutil sama kiirus
        ...
        horizontalInputYaw = Input.GetAxis("HorizontalYaw");

        //Siin paneme info oma muutujasse
        yaw += horizontalInputYaw * Time.deltaTime * 50;
        ...
        mainCameraGuidePosition.Rotate(0, horizontalInputYaw * Time.deltaTime * 50, 0);

        //Siin me muudame mängija kiirust
        thrust += Input.mouseScrollDelta.y;
